 ...
 * Installer
 * 
 * Uses synchronous recursion to install app dependencies
 */
module.exports = {
    ...
    _installPackages : function (opts) {
        ...
        var jsonPath, appJsonPath, iter;

        jsonPath = (opts.path) ? opts.path.concat('/package.json') : ('app.json');
        appJsonPath = path.resolve(jsonPath);
        var data = {},
            // appJsonPath = this.App.getPackageJsonPath(jsonPath), 
            that = this;
        data.appConfig = opts.appConfig || this.App.loadAppProperties(appJsonPath);
    ...
    _installPackage : function (opts, fn) {
        ...
        this._getPackageInfo(opts, function (err, packageInfo) {
            ...
            if (Git.gitCloneRepo(data) !== 0) {
                ...
                var branch = Git.getBestBranch(data);
                if (branch !== "master") Git.gitCheckoutBranch({path : data.dest, branch : branch});
    ...
    _tracePackageDependencies : function (opts) {
        ...
        if (fs.existsSync(opts.packagePath + '/app.json')) {
            var appProperties = this.App.loadAppProperties(opts.packagePath + '/app.json');
